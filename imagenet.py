...
def train(net, train_loader, optimizer):
    ...
    for i, (images, targets) in enumerate(train_loader):
        ...
        output, loss = net(images, targets, cycles=2, step=0.1, mse_parameter=0.1, kl_parameter=0.1, tv_parameter=0.1)
        loss = loss.mean()
...
def main():
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        k = args.epochs // 3
        if epoch == k - 1 or epoch == 2 * k - 1:
            shutil.copyfile(save_path, os.path.join(args.save, 'model_epoch_%i.pth.tar' % epoch))
